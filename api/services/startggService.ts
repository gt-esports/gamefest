...
const client = axios.create({
  ...
  headers: {
    ...
    Authorization: env.startgg.apiToken
      ? `Bearer ${env.startgg.apiToken}`
      : undefined,
  },
});
...
export async function fetchTournamentBySlug(
  ...
  const response = await client.post<
    StartggGraphQLResponse<{ tournament: Tournament | null }>
  >("", { query, variables: { slug } });

  if (response.data.errors?.length) {
    throw new Error(
      `start.gg error: ${response.data.errors.map((e) => e.message).join(", ")}`
    );
  }
...
export async function fetchTournamentEventsBySlug(slug: string): Promise<{
  ...
  >("", { query, variables: { tourneySlug: slug } });
...
export async function fetchEventWithPhases(params: {
  ...
  phases: Array<{
    id: string | number;
    name: string;
    phaseGroupIds: Array<string | number>;
  }>;
} | null> {
  if (!env.startgg.apiToken) return null;
  ...
  const response = await client.post<
    StartggGraphQLResponse<{
      event: { phases: EventPhase[] | null } | null;
    }>
  >("", { query, variables: params });
